use crate::ski_lang::{SkiLang, SkiLangM44, SkiLangRect};
use egg::*;

use crate::protos::{
    sk_paint::Blender, sk_paint::ColorFilter, sk_paint::ImageFilter, sk_paint::MaskFilter,
    sk_paint::PathEffect, sk_paint::Shader, BlendMode, Bounds, SkColor, SkM44, SkPaint,
};
...
pub fn paint_proto_to_expr(expr: &mut RecExpr<SkiLang>, skPaint: &Option<SkPaint>) -> Id {
    let color = match &skPaint {
        Some(skPaint) => {
            match &skPaint.color {
                Some(skColor) => color_proto_to_expr(
                    expr,
                    skColor.alpha_u8,
                    skColor.red_u8,
                    skColor.green_u8,
    ...
    let blender = match &skPaint {
        Some(skPaint) => match &skPaint.blender {
            Some(blender) => {
...
pub fn bounds_expr_to_proto(expr: &RecExpr<SkiLang>, id: Id) -> Option<Bounds> {
    let bounds: Option<Bounds> = match &expr[id] {
        SkiLang::Bounds(ids) => match &expr[ids[0]] {
            SkiLang::Bool(true) => Some(unpack_rect_to_bounds(&expr, ids[1])),
            SkiLang::Bool(false) => None,
...
pub fn paint_expr_to_proto(expr: &RecExpr<SkiLang>, id: Id) -> SkPaint {
    ...
    SkPaint {
        ...
        // at this point.
        blender: Some(Blender { blend_mode }),
        ...
        shader: if shader_exists { Some(Shader {}) } else { None },
    }
}
